...
<xsl:template match="d:para[@role = 'statute']|d:paranum">
  ...
  <xsl:template match="d:para[@role = 'statute']|d:paranum" mode="xref-to">
    ...
    <xsl:choose>
      ...
      <xsl:when test="parent::d:section">
        <xsl:apply-templates select="parent::d:section" mode="label.markup"/>.<xsl:number from="d:section" count="d:para[@role='statute']|d:paranum|d:section" level="single"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="d:subline">
    ...
    <fo:inline-container>
      ...
      <fo:block text-indent="0mm" text-align="center" text-align-last="center" font-size="12pt" border-top-width="0.2mm" border-top-style="solid">
        <xsl:attribute name="width">
          <xsl:choose>
            <xsl:when test="$width"><xsl:value-of select="$width"/></xsl:when>
            <xsl:otherwise>5cm</xsl:otherwise>
  ...
  <xsl:template match="d:underline">
    ...
    leader-alignment="reference-area">_</fo:leader>
  </xsl:template>
  ...
  <xsl:template match="d:twoblock">
    <fo:block text-align-last="justify">
      <fo:inline><xsl:apply-templates select="d:left"/></fo:inline>
      <fo:leader leader-pattern="space" leader-length.optimum="100%"
                 leader-alignment="reference-area"/>
      <fo:inline><xsl:apply-templates select="d:right"/></fo:inline>
    </fo:block>
  </xsl:template>
