...
<xsl:template name="itemizedlist.label.markup">
    <xsl:param name="itemsymbol" select="'disk'"/>
-
</xsl:template>
...
<xsl:attribute-set name="orderedlist.properties">
  <xsl:attribute name="text-indent">
    <xsl:choose>
      <xsl:when test="ancestor::d:listitem">25mm</xsl:when>
...
<!--    </xsl:call-template>-->
    <xsl:choose>
      <xsl:when test="parent::d:orderedlist/@numeration"><xsl:value-of select="parent::d:orderedlist/@numeration"/></xsl:when>
